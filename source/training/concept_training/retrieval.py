...
def retrieve(target_name, outpath, num_class_images):
    if ClipClient is None:
        raise ImportError(
            "Missing optional dependency `clip-retrieval`. Install it to use real prior retrieval ("
            "e.g. `pip install clip-retrieval`), or disable `--real_prior`/provide your own class images."
        )
