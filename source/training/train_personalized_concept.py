...
sys.path.append('./')
from concept_training.model_pipeline import CustomDiffusionAttnProcessor, set_use_memory_efficient_attention_xformers
from concept_training.data_pipeline import CustomDiffusionDataset, PromptDataset, collate_fn
from concept_training.model_pipeline import CustomDiffusionPipeline
from concept_training import retrieval
            ...
            if args.real_prior:
                if accelerator.is_main_process:
                    if not Path(os.path.join(class_images_dir, 'images')).exists() or len(list(Path(os.path.join(class_images_dir, 'images')).iterdir())) < args.num_class_images:
                        retrieval.retrieve(concept['class_prompt'], class_images_dir, args.num_class_images)
...
#                 prompt_embeds = prompt_embeds.hidden_states[-2]
    ...
    # Save a final checkpoint at the end of training even if max_train_steps is not a multiple of save_steps.
